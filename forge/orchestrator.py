...
def run_specialized_pipeline(repo: str | Path, output_dir: str | Path, max_connected: int = 100) -> dict[str, Any]:
    ...
    skill_run = run_skills(triage_manifest)
    ...
    findings += list(skill_run.findings)
    ...
    skills_path = out / "skills-runtime.json"
    ...
    return {"repo": str(root), "connected_alive": connected, "findings": len(findings), "coverage": coverage.to_dict(), "artifacts": {"triage": str(triage_path), "hypotheses": str(hypotheses_path), "verification": str(verification_path), "sealed": str(sealed_path), "coverage": str(coverage_path), "skills": str(skills_path), "report": str(report_path)}}
