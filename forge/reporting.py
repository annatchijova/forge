...
def render_sharded_dashboard(run_dir: str | Path, destination: str | Path | None = None) -> Path:
    ...
    cards = []
    total_findings = total_discarded = 0
    for item in plan.get("shards", []):
        ...
        tone = "ok" if status == "COMPLETE" else "fail"
        cards.append(
            ...
            f"<a class='button' href='shards/shard-{index:04d}/forge-report-standard.html'>Open standard report</a> "
...
<div class='stats'><div class='stat'><strong>{total_findings}</strong><span>surviving leads across shards</span></div><div class='stat'><strong>{total_discarded}</strong><span>discarded hypotheses</span></div><div class='stat'><strong>{len(cards)}</strong><span>independently sealed shards</span></div></div>
