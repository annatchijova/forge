...
def render_tiered_report(sealed_path: str | Path, mode: str, destination: str | Path | None = None) -> Path:
    ...
    disposition_status = metrics.get("audit_disposition", {}).get("status")
    if not disposition_status:
        disposition_status = "VERIFIED" if verification.get("ok") else "FAILED"
...
*{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--ink);font:16px/1.55 var(--sans);-webkit-font-smoothing:antialiased}.wrap{max-width:1180px;margin:0 auto;padding:0 24px 56px}header{border-bottom:2px solid var(--ink);padding:40px 0 22px;margin-bottom:36px}h1{font:600 clamp(28px,4vw,40px) var(--serif);margin:0 0 18px}h2{font:600 21px var(--serif);color:var(--accent);border-bottom:1px solid var(--rule);padding-bottom:10px}h3{font:600 17px var(--serif);margin-bottom:6px}section{background:var(--bg-elevated);border:1px solid var(--rule);border-radius:3px;margin:24px 0;padding:22px 24px;box-shadow:0 4px 14px rgba(28,34,34,.06)}#seal p{display:inline-block;background:var(--bg-sunken);color:var(--ink-muted);border:1px solid var(--rule);border-radius:14px;padding:6px 12px;font:12px var(--mono);letter-spacing:.04em}#seal p.status-ok{background:#DFEDE2;color:#2A5A3C;border-color:#A9C9B0}#seal p.status-partial{background:#FFF4E9;color:#7A3A14;border-color:#D89A70}#seal p.status-fail{background:#F8E5E5;color:#8B2F2F;border-color:#D39A9A}.finding{background:var(--bg);border:1px solid var(--rule);border-left:4px solid var(--accent);border-radius:3px;padding:14px 18px;margin:14px 0}.finding p:first-of-type{color:var(--ink-muted);font:12px var(--mono)}pre{background:var(--bg-sunken);border-radius:3px;padding:12px 14px;overflow:auto;white-space:pre-wrap;font:13px/1.5 var(--mono)}#limitations h2,#discarded h2{color:var(--fail)}
</style>
"""
