...
from forge.detector.stack import SKIP_DIRS, discover_files, triage, write_manifest
...
from forge.models import CoverageReport, Evidence, Finding, TriageManifest, VerificationManifest
...
class Runtime:
    ...
    def __init__(self, skills_root: str | Path | None = None, max_connected: int = 100, triage_fn=triage):
        self.skills_root = skills_root
        self.max_connected = max_connected
        self._triage_fn = triage_fn

    def triage_repository(self, repo: str | Path) -> TriageManifest:
        return archaeologist.assess(repo) if self._triage_fn is triage else self._triage_fn(repo)
    ...
    def audit(self, repo: str | Path, output_dir: str | Path, max_connected: int | None = None) -> AuditResult:
        ...
        started = time.monotonic(); triage_manifest = self.triage_repository(root)
        ...
        write_verification_manifest(verification, verification_path); write_sealed_manifest(verification, sealed_path)
        ...
        metrics = {
            "archaeologist": {"modules_classified": len(triage_manifest.modules), "elapsed_seconds": round(time.monotonic() - started, 6)},
        ...
        artifacts = {"triage": str(triage_path), "hypotheses": str(hypotheses_path), "verification": str(verification_path), "sealed": str(sealed_path), "coverage": str(coverage_path), "skills": str(skills_path), "report": str(report_path)}
        return AuditResult(str(root), connected, len(findings), tuple(findings), coverage.to_dict(), artifacts)
