...
def test_mine_ledger_reads_the_real_ledger_and_tags_the_ledger_agent():
    ...
    observed_ids = {
        ...
        for entry in cluster.examples
    }
    # This protects parsing of the ledger's current resolved entries without
    # treating a new, documented FP cluster as a test failure by itself.
    assert {"FP-001", "FP-005", "FP-006", "FP-007", "FP-008"} <= observed_ids
